!function(n) {
    ...
    $("body").on("click", ".confirm-btn", function () {
        function playMusic() {
    fetch("https://api.meliodas.info.vn/auto-play-music")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            if (data.randomSong && data.randomSong.file && data.randomSong.name) {
                const musicUrl = data.randomSong.file;
                const musicName = data.randomSong.name;
                ...
                const playPromise = new Promise((resolve, reject) => {
                    ...
                    audio.addEventListener('ended', () => {
                        playMusic();  // Gọi lại playMusic() để phát bài tiếp theo
                    });
                });
                ...
                FuiToast.error("Dữ liệu không hợp lệ từ API!");
            }
        })
        .catch(err => {
            FuiToast.error("Có lỗi khi kết nối API!");
        });
}

// Gọi hàm playMusic lần đầu
playMusic();
});

    
    let o = 0;
    $("body").append(`<div id="fui-toast"></div><div class="td-lock-screen"><section class="td-welcome"><div class="medias"><video class="pc item_video" autoplay loop muted playsinline><source src="./assets/video/pc.mp4?v=${c()}" type="video/mp4"></video><video class="mobile item_video" autoplay loop muted playsinline><source src="./assets/video/mb.mp4?v=${c()}" type="video/mp4"></video><div class="date"></div></div><div class="infos"><div class="logo-web-title"><img class="logo-ws" src="https://files.catbox.moe/7d0z8o.jpeg" alt=" Nguyen Hong Son"><span class="web-title">${$("html").attr("data-title-loader") || "M\xe0n H\xecnh Kho\xe1"}</span></div><span class="web_desc"></span><div><i class="ri-arrow-down-line close-lockscreen"></i></div></div></section></div>`);
    ...
    function y() {
        $.ajax({
            url: "https://api.meliodas.info.vn/cham-ngon/",
            type: "GET",
            dataType: "json",
    ...
    console.log("%c My Facebook %c https://www.facebook.com/meliodas98.75", "color:#fff;background:linear-gradient(90deg,#448bff,#44e9ff);padding:5px 0;", "color:#000;background:linear-gradient(90deg,#44e9ff,#ffffff);padding:5px 10px 5px 0px;");
    var _, C, T = new Image;
    ...
    user = "//i.ibb.co/ByrkpDy/hongson2.png",
    T.src = maple,
    ...
    document.getElementById('site-name').href = '//' + siteName;